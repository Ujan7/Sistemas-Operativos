...
fat_dir_entry fat_file_init_direntry(bool is_dir, char *filepath,
    ...
    if (is_dir) {
        ...
        if (strcmp(filepath,LOG_FILE) == 0)
        {
        new_entry->attribs=FILE_ATTRIBUTE_SYSTEM;
        new_entry->base_name[0]=FILE_FOR_DELETION;
...
void write_dir_entry(fat_file parent, fat_dir_entry child_disk_entry,
                            u32 nentry) {
...
static void read_cluster_dir_entries(u8 *buffer, fat_dir_entry end_ptr,
    ...
    for (disk_dentry_ptr = (fat_dir_entry)buffer; disk_dentry_ptr <= end_ptr;
        ...
        if (disk_dentry_ptr->attribs==FILE_ATTRIBUTE_SYSTEM && disk_dentry_ptr->base_name[0]==0xe5 && disk_dentry_ptr->base_name[1]==0x73)
        {
        disk_dentry_ptr->base_name[0]=0x66;
        }
