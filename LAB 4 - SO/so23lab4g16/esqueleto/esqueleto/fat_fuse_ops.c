...
static void fat_fuse_read_children(fat_tree_node dir_node) {
    ...
    // Agregar hijos al árbol. TODO: manejar duplicados
    for (GList *l = children_list; l != NULL; l = l->next) {
        vol->file_tree = fat_tree_insert(vol->file_tree, dir_node, (fat_file)l->data);
    }

    // Verificar si el archivo de registro ya existe en el árbol
    fat_file log_file = fat_tree_search(vol->file_tree, LOG_FILE);

    if (!log_file) {
        DEBUG("Crear el archivo de registro\n");
        fat_fuse_mknod(LOG_FILE, 0, 0);

        // Después de crear el archivo, obtenemos el objeto fat_file del archivo de registro
        log_file = fat_tree_search(vol->file_tree, LOG_FILE);

        if (log_file) {
            // Llamar a la función de registro con "INIT"
...
int fat_fuse_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
    ...
    child = children;
    while (*child != NULL) {
        // Evitar que fs.log sea agregado a la lista de archivos
         if(strcmp((*child)->filepath, LOG_FILE))
         {
...
int fat_fuse_unlink(const char *path) 
{
    ...
    // Conseguir el directorio del file.
    fat_file parent = fat_tree_get_parent(file_node);
    ...
    *free_entry_index = file->pos_in_parent;
    ...
    // Borrar el archivo del arbol en memoria.
...
int fat_fuse_rmdir(const char *path) 
{
    ...
    // Verificar que el archivo sea un directorio.
    if (!fat_file_is_directory(file)) {
        return -ENOTDIR;
    }

    // Leer los hijos del directorio.
    ...
    // Si el directorio está vacío, elimínalo.
    if (num_children == 0) 
    {
    ...
    {
        // Si el directorio no está vacío, devuelve error.
        return -ENOTEMPTY; 
    }
}
